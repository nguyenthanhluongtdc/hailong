...
git clone ssh://git@git.visualweber.net:1506/viweb-dev/hailongglass.git -b develop
cd /home/dev/htdocs/hailongglass/
touch README.md
git add README.md
...
cd /home/dev/htdocs/hailongglass/
git init
git remote add origin ssh://git@git.visualweber.net:1506/viweb-dev/hailongglass.git
git fetch --all
...
```
<VirtualHost *:80>
    ServerAdmin admin@hailongglass.dev.gistensal.com
    ServerName hailongglass.dev.gistensal.com
    ServerAlias www.hailongglass.dev.gistensal.com
    SetEnv APPLICATION_ENV "development"
    SetEnv ENVIRONMENT "development"
    SetEnv ENV "development"

    ErrorLog "/home/dev/logs/hailongglass-error_log"
    DocumentRoot "/home/dev/htdocs/hailongglass/"
    <Directory "/home/dev/htdocs/hailongglass/">
...
##### Domain: http://hailongglass.dev.gistensal.com or https://hailongglass.dev.gistensal.com
...
`/home/dev/htdocs/hailongglass/`

Just run this file, then you will have the delivery files

```
$ bash /home/dev/htdocs/hailongglass/bin/delivery.sh
```

- http://hailongglass.dev.gistensal.com/hailongglass.zip 
- http://hailongglass.dev.gistensal.com/hailongglass.sql 
- Done!
...
- Mysql DB Name: dev_hailongglass
- Mysql DB User: userdb.dev.hailongglass
- Mysql DB Password: qtevq2woJk0sXrWC
```
##### MongoDB Database:

```
- Mongo DB Name: dev_hailongglass
- Mongo DB User: userdb.dev.hailongglass
- Mongo DB Password: qtevq2woJk0sXrWC
```
...
```
/usr/bin/certbot delete --cert-name hailongglass.dev.gistensal.com
```
...
```
sudo certbot --authenticator standalone --installer nginx -d hailongglass.dev.gistensal.com -d www.hailongglass.dev.gistensal.com --pre-hook "service nginx stop" --post-hook "service nginx start"
```
...
IMPORTANT NOTES:
 - Congratulations! Your certificate and chain have been saved at
   /etc/letsencrypt/live/hailongglass.dev.gistensal.com/fullchain.pem. Your cert will
...
```
zip -r hailongglass.zip . -x \*.buildpath/\* \*.idea/\* \*.project/\* \*nbproject/\* \*.git/\* \*.svn/\* \*.gitignore\* \*.gitattributes\* \*.md \*.MD \*.log \*.zip \*.tar.gz \*.gz \*.tar \*.rar \*.DS_Store \*.lock \*desktop.ini vhost-nginx.conf \*.tmp \*.bat bin/delivery.sh bin/remove-botble.sh readme.html composer.lock wp-config.secure.php

-- without symlink
zip -r4uy hailongglass.zip . -x \*.buildpath/\* \*.idea/\* \*.project/\* \*nbproject/\* \*.git/\* \*.svn/\* \*.gitignore\* \*.gitattributes\* \*.md \*.MD \*.log \*.zip \*.tar.gz \*.gz \*.tar \*.rar \*.DS_Store \*.lock \*desktop.ini vhost-nginx.conf \*.tmp \*.bat bin/delivery.sh bin/remove-botble.sh readme.html composer.lock wp-config.secure.php
```
...
```
mysqldump  -hlocalhost -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC dev_hailongglass > dev_hailongglass.sql
mysqldump  -uuserdb.dev.hailongglass -p dev_hailongglass > dev_hailongglass.sql
mysqldump  -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC dev_hailongglass | gzip -9 > dev_hailongglass.sql.gz
mysqldump  -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC --default-character-set utf8 dev_hailongglass  > dev_hailongglass.mysql
```

* Restoring your MySQL Database

```
mysql -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC dev_hailongglass < dev_hailongglass.sql
mysql -h127.0.0.1 -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC --default-character-set utf8 dev_hailongglass < dev_hailongglass.sql


If you need to restore a database that already exists, you'll need to use mysqlimport command. The syntax for mysqlimport is as follows
mysqlimport -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC dev_hailongglass < dev_hailongglass.sql

To restore compressed backup files you can do the following:
gunzip < dev_hailongglass.sql.gz | mysql -uuserdb.dev.hailongglass -pqtevq2woJk0sXrWC dev_hailongglass

Restore from FILE:
mysql.exe --defaults-file="c:\users\anonym~1\appdata\local\temp\tmpn3ytli.cnf"  --protocol=tcp --host=127.0.0.1 --user=userdb.dev.hailongglass --port=3306 --default-character-set=utf8 --comments  < "E:\\dev_hailongglass.sql"
...
FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = dev_hailongglass;
```
...
sudo mongodump --host localhost:27017 -u userdb.dev.hailongglass -p qtevq2woJk0sXrWC --db dev_hailongglass --out mongodb-2017-04-09.json
```

* Restore your MongoDB Database

```
sudo mongorestore --host localhost:27017 -u userdb.dev.hailongglass -p qtevq2woJk0sXrWC --db dev_hailongglass --drop /home/jobsin/mongodb-2017-04-09.json/dev_hailongglass
```
...
$ use dev_hailongglass
$ db.createUser( { user: "userdb.dev.hailongglass",pwd: "qtevq2woJk0sXrWC",roles: [ {role:"readWrite",db:"dev_hailongglass"}] } )
```
